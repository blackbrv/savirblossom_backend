...
import { usePromo, useUpdatePromo } from "@/services/Promos/PromosApi";
...
export default function PromoEdit() {
    ...
    const { data: promoData, isLoading: isFetching } = usePromo(promoId);
    ...
    React.useEffect(() => {
        if (promoData?.data) {
            ...
            setValue("scheduled_at", formatDateTimeInput(promo.scheduled_at));
            setValue("cta_label", promo.cta_label ?? "");
            setValue("cta_url", promo.cta_url ?? "");
        }
    }, [promoData, setValue]);
